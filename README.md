...
stayed link-only (no `file_picker` dependency to let a user pick a local file). Verified locally
(backend: `npm install`, `tsc`, `eslint`, `nest build`, and `npm test` all pass — 213 tests, plus
`npm run test:e2e` 7/7 against real Postgres; mobile: hand-verified import paths, not yet run
