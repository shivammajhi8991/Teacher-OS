...
GitHub Actions CI pipeline for both `backend/` and `mobile/`), step 2 (a security review pass
against docs/04 §4.8 + OWASP API/Mobile Top 10), step 3 (load testing attendance-bulk-mark and
invoice-generation), and step 4 (self-service account deletion & data export) are done; App
Store/Play Store submission and staged rollout remain genuinely blocked on external developer
accounts and credentials this environment cannot obtain — see docs/07-roadmap.md's Phase 6 step 5
entry for exactly what's been prepared regardless (a full data-handling map, a rollout plan) versus
what a human still needs to do. Setting up mobile CI meant installing a real Flutter SDK for the
first time in this project's history (previously never available in this dev environment — every
mobile module across Phase 4–5 was hand-verified for Dart syntax instead), which immediately found
and fixed 4 real, previously-undiscoverable bugs — see docs/07-roadmap.md's Phase 6 step 1 entry.
Load testing (step 3) found a real single-process concurrency bottleneck — confirming, not just
theorizing, docs/02 §2.9's existing "horizontal-scale API pods" scale path — and building account
deletion (step 4) found a real bug of its own: a soft-deleted user's email/phone had stayed
permanently unusable since Phase 4 step 1, now fixed. Verified locally: backend `npm install`,
`tsc`, `eslint`, `nest build`, and `npm test` all pass — 265 tests, plus `npm run test:e2e` 7/7
against real Postgres + Redis; mobile `flutter analyze` → no issues, `flutter test` → all passing,
both against a real Flutter 3.47.2 SDK (see [mobile/README.md](mobile/README.md)).
...
**Phase 5 (Advanced features) is complete** — all 8 steps (Assignments, Performance tracking, Parent dashboard, the Institute/admin module incl. announcements, Reports & analytics, Calendar unification, CSV import, and the Admin web panel) are done; see [docs/07-roadmap.md](docs/07-roadmap.md) for the full step-by-step narrative. **Phase 6 (Testing & production deployment) is in progress** — step 1 (CI pipeline), step 2 (security review), step 3 (load testing), and step 4 (self-service account deletion & data export) are done; App Store/Play Store submission and staged rollout remain genuinely blocked on external developer accounts/credentials this environment cannot obtain (see docs/07-roadmap.md's Phase 6 step 5 for the data-handling map and rollout plan already prepared regardless). Worth noting a few things the next pass should know about: Fees' offline behavior stayed on Attendance's simpler "always converges" policy rather than building the fuller "financial edits never auto-merge, dedicated conflict-resolution screen" policy docs/05 §5.4 describes — `POST /payments` isn't wired into the mobile offline queue at all yet (a payment while offline currently just fails with a network error rather than queuing). Notes and Assignments are both mobile-scoped to link-only content, and CSV import (step 7) has no mobile screen at all, for the same underlying reason: a real file-upload/download/pick UI needs new pubspec dependencies (`file_picker`, and a way to open a downloaded file) not yet pulled into this pass; the backend supports the full upload/version/download flow for Notes/Assignments already (via a storage adapter shared between the two modules, `backend/src/common/storage/`), and CSV import's `POST /students/import` is likewise real and fully usable from any HTTP client, just not from the app yet. Notifications' push delivery is real-but-mocked (no Firebase project exists) and mobile never registers a device token (needs `firebase_messaging` + real platform config); digest batching still runs on an in-process cron rather than BullMQ — there's still no BullMQ queue in this codebase, though Redis itself is no longer unused now that Phase 6's rate limiter is genuinely backed by it (`backend/src/common/throttler/`) — the same BullMQ gap is why Reports' and CSV import's async-job paths (steps 5 and 7) both run their background work as a fire-and-forget call in-process rather than a real queued job. Reports is also this project's first real new backend dependency since the initial scaffold (`pdfkit`, for actual PDF generation) — CSV stays hand-rolled, in both directions (Reports' writer, CSV import's parser). The Admin web panel (Phase 5 step 8) is likewise not a literal second Flutter Web build target — a real Flutter SDK now exists in this environment (installed for Phase 6 step 1's CI work) but has still never run `flutter create --platforms web .` to scaffold one — the admin panel is just that target's presentation-layer source, hosted for now at the `/admin-panel` mobile route. `audit_logs` (docs/03 §3.10) was discovered during Phase 5 step 8 to have never actually been built beyond seeding its own permission back in Phase 4 step 1 — worth picking up if Phase 6 needs a real audit trail; `npm audit`'s 28 pre-existing dependency vulnerabilities (Phase 6 step 2's own finding) are worth their own dedicated upgrade pass too, rather than a drive-by `--force` fix that risks this project's `@nestjs/*` version alignment. Docker is usable in this environment — `docker compose -f infra/docker-compose.yml up -d && npm run migration:run` in `backend/` gets a real Postgres **and** Redis up (host port **5433** for Postgres, not 5432 — see that compose file's comment) — and `npm run test:e2e` is worth running after any auth/RBAC change, now that it actually can be — it, and manually exercising the live API, have together caught seventeen real backend bugs so far across Phase 4–6 (see backend/README.md's own numbered list), plus a handful of mobile-only ones (the super_admin router misroute in Phase 5 step 8, and three more Phase 6 step 1 found the moment a real Flutter SDK ran against this codebase for the first time — see the Status section above and docs/07-roadmap.md).
